...
fork:
    sub eax, 1 ; e un fel de for ( k = N; k >= 0; k--) in care iau patratele
	; fiecare patrat are dimensuiune k
	cmp eax, 0 ; verific daca am ajuns la elementul 0
    je odd_nr ; fac verificarea pe caz a lui k si N. aici verific sa vad daca N e impar sau nu
...
c_k:
    ...
    add ebx, 1 ; trec mai departe in textul in clar
...
continue:
    ...
    sub eax, 1 ; scad din dimensiunea unui patrat 1 ca se micsoreaza patratele in mod progresiv
    cmp eax, 0
    jg fork
    jle exit
odd_nr:
...
copy:
    ...
    add ebx, 1 ; trec la urmatorul caracter din clarul modificat
    add edx, 1 ; la fel si in ceea ce pun ce am modificat
    dec ecx
